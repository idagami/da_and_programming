...
def save_info():
    ...
    if len(entry_password.get()) < 10:
        ...
        try:  # open file
            with open(
                "hasla.json", mode="r"
            ) as passwords_file:  # to update existing file, 3 steps
                data_read = json.load(passwords_file)
            ...
            with open("hasla.json", mode="w") as passwords_file:
                json.dump(data_read, passwords_file, indent=4)  # now saving the data
        finally:
            label_status.config(
...
def search_data():
    website_input = entry_web.get().lower()
    cur_dir = os.path.dirname(__file__)
    json_dir = os.path.join(cur_dir, "hasla.json")
    try:
        ...
        try:
            password_found = json_data[website_input]["password"]
            window_program.clipboard_clear()
            window_program.clipboard_append(password_found)
            window_program.update()
            messagebox.showinfo(
                title=website_input,
                message=f"Following data found:\nUser/email: {entry_user.get()},\nPassword: {password_found}\nPassword copied to clipboard",
            )
        except KeyError:
            messagebox.showerror(
                title="Error",
                message="Requested website not found in the password vault",
            )
...
logo_path = os.path.join(cur_file_dir, "logo.png")
