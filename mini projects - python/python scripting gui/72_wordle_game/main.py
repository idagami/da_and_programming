...
class Wordle:
    ...
    def __init__(self):
        ...
        self.root.wm_iconbitmap(os.path.join(BASE_DIR, "images", "icon.ico"))
        ...
        self.setting = Image.open(os.path.join(BASE_DIR, "images", "setting.png"))
        self.setting = self.setting.resize((40, 40), Image.Resampling.LANCZOS)
        self.setting = ImageTk.PhotoImage(self.setting)

        self.setting_dark = Image.open(
            os.path.join(BASE_DIR, "images", "setting_dark.png")
        )
        ...
        label = Image.open(os.path.join(BASE_DIR, "images", "head.png"))
    ...
    def show_popup(self):
        ...
        print(os.path.exists(os.path.join(BASE_DIR, "images", "icon.ico")))
        popup.wm_iconbitmap(os.path.join(BASE_DIR, "images", "icon.ico"))
        popup.focus_force()
    ...
    def change_keypad_color(
        self, color, guess, on_hover_color=None, off_hover_color=None
    ):
        for char in guess:
            if 65 <= ord(char) <= 70:
                btn_frame_index = 0
    ...
    def get_from_db(self):
        ...
        cursor.execute("SELECT * FROM info WHERE id = 0")
        data = cursor.fetchone()

        if data is None:
            cursor.execute(
                "INSERT INTO info (id, word_length, high_score) VALUES (0, ?, ?)",
                (5, 0),
            )
            self.word_size = 5
            self.high_score = 0
        else:
            self.word_size = data[1]
            self.high_score = data[2]
